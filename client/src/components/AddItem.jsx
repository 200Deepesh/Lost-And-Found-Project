          ...
          <div className='flex gap-4'>
            <div id='left' className='w-fit flex flex-col gap-2'>
              ...
              <div className='w-full border-2 border-[#D9D9D9] box-border rounded-xl p-2 flex flex-col gap-2'>
                ...
                <div className='relative'>
                  <select name='sem' onChange={(e) => { setStudentInfo({ ...studentInfo, [e.target.name]: e.target.value }); console.log(studentInfo) }} className='w-full max-w-40 border border-[#D9D9D9] rounded-full py-1 px-2 text-[12px]  focus-visible:outline-0 focus-visible:border-black' style={{ color: studentInfo.sem ? 'black' : '#00000066' }}>
            ...
            <div id='right' className='flex flex-col gap-2'>
              <div className='relative'>
                <input type="text" name='name' value={itemInfo.name} onChange={(e) => { setItemInfo({ ...itemInfo, [e.target.name]: e.target.value }) }} placeholder='Item Name' className='w-56 border border-[#D9D9D9] rounded-full py-1 px-2 text-[12px]  focus-visible:outline-0 focus-visible:border-black placeholder:text-[12px] placeholder:text-[#00000066] ' />
              ...
              <div className='relative'>
                <input type="text" name='location' value={itemInfo.location} onChange={(e) => { setItemInfo({ ...itemInfo, [e.target.name]: e.target.value }) }} placeholder='Location' className='w-56 border border-[#D9D9D9] rounded-full py-1 px-2 text-[12px]  focus-visible:outline-0 focus-visible:border-black placeholder:text-[12px] placeholder:text-[#00000066] ' />
              ...
              <div className='relative'>
                <input type="date" name='date' value={itemInfo.date} onChange={(e) => { setItemInfo({ ...itemInfo, [e.target.name]: e.target.value }) }} placeholder='Location' className='w-56 border border-[#D9D9D9] rounded-full py-1 px-2 text-[12px]  focus-visible:outline-0 focus-visible:border-black placeholder:text-[12px] placeholder:text-[#00000066] ' />
              ...
              <div className='flex gap-0.5 flex-col relative'>
                ...
                <textarea name="discription" id="" value={itemInfo.discription} onChange={(e) => { setItemInfo({ ...itemInfo, [e.target.name]: e.target.value }) }} placeholder='Discription the item...' className='w-84 h-48 border-2 box-border border-[#D9D9D9] rounded-xl p-2 text-xs resize-none focus-visible:outline-0 focus-visible:border-black placeholder:text-[12px] placeholder:text-[#00000066] overflow-auto'></textarea>
              ...
              <div className='border-2 box-border border-[#D9D9D9] p-2 w-84 rounded-xl gap-1 flex flex-col'>
                ...
                <div className='w-full flex gap-1 flex-wrap'>
                  {preDefineTags.map((tag) => <QuickSearchTags key={tag} tagName={tag} tags={itemInfo.quickSearchTags} setTag={(newTags) => { setItemInfo({ ...itemInfo, quickSearchTags: newTags }) }} />)}
                </div>
              </div>
            </div>
          </div>
          <div className='flex items-center justify-center'>
            <button className='bg-[#050506CF] rounded-full px-4 py-1 text-white text-xs font-poppins'>Submit Form</button>
          </div>
        </form >
      </div>
    </>
